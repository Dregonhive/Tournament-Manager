...
void simulMatch( Equipe *team1, Equipe *team2, Resultats tab[],int cptr ,char* phase)
{
    ...
    int Forfait = rand()%(20 + cptr);  // Cas tres rare d'un forfait
    if (Forfait == 0)
    {
        team1 -> points += 2;
        char note[60] = "Forfait de l'equipe : ";
...
// @param nb_match = nombre de match jouer jusqu'a present, sert en tant qu'indice du tableau des resulats
{
...
void permutEq(Equipe**tete ,Equipe**queue, Equipe *eq1, Equipe *eq2 ,  int * nb_chain)
{
    ...
    int cpt=0;//l'indicateur des permutation
...
void fusion_poul( Equipe**tete1,Equipe** queue1 , Equipe**tete2 ,Equipe** queue2 , int*nb_chain)
{
    ...
    q1=*queue1;
    *queue1=(*tete2)->next;
    affAdrDB(*queue1 , NULL);//(*queue1)->next=NULL;
    p=(*tete1)->next;
    affAdrDB(p,*tete2); //p->next = *tete2;
    affAdrGB(*tete2 ,p);//(*tete2)->prev=p;
    (*tete2)=NULL;
    nb_chain+=3;
...
void deroulement_post_poule(Equipe** tete , Equipe** queue , Resultats tab[] , int* nb_match , int *match_tournois , int* nb_eq_allou ,int *nb_chain)//deroulement des matche dans une poule apres la phase eliminatoire         team->tete = tete; team->queue = tail;
{
    Equipe*p = (*tete)->next;
    Equipe*q = (*queue)->prev;
    ...
    *match_tournois=(*match_tournois) +1;
    ...
    //réinitialisation de la tete et la queue
...
void organisation_tournois(Poule* tete_tournois ,int *nb_chain)//assure l'organisation du tournois en poulede 4 element chacune apres la phase eliminatoire
{
    Poule*p;
    Poule*old;
    Poule*q;
...
void fusion_post_poul(Equipe**tete1,Equipe** queue1 , Equipe**tete2 ,Equipe** queue2 ,int*nb_chain) //fusionne lees poules apres la phase eliminatoire
{
    //chainage
    affAdrDB(*queue1,*tete2);//(*queue1)->next = *tete2;
    affAdrGB(*tete2,*queue1);//(*tete2)->prev = *queue1;
    *nb_chain+=2;
    *queue1=*queue2;
    //liberation
    *queue2=NULL;
...
//ce module assure le deroulement des matches dans les poules utilsant la regle d'organiser les affrontements en nombre pair et impair.
{                                                   /*np = numbre de poule*/
    Equipe*p = (*tete)->next;
    Equipe*q = (*queue)->prev;
